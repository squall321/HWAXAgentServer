...
_CHAIR_ITEMS = {
    # ⚠ 종전 default 는 (1)부터 '결정사항(번호매김)'이라 문서 전체가 항목 목록 + 그 목록에
    #   대한 주석이 됐다. "무슨 일이 벌어지고 있는가"를 적을 자리가 아예 없었다 — 결과가
    #   지엽적으로 나온 건 모델 탓이 아니라 계약이 그렇게 짜여 있어서다. 전문 템플릿
    #   (diagnosis·option-select·credibility·mechanism)은 전부 (1)에서 문제를 세우고 시작한다.
    #   머리 절(상황 판단)·관계 절·축 구분·뒤집힘 조건을 넣어 그 결손을 메운다.
    #   hwax-deliberate.js CHAIR_ITEMS.default 와 같은 내용을 유지한다(경로별 결과 어긋남 방지).
    "default":
        "(1) 상황 판단 — 지금 무엇이 문제이고 무엇이 걸려 있는가를 3~6문장의 하나의 그림으로. "
        "아래 항목을 줄여 옮긴 요약이 아니다. 무엇이 이 판을 어렵게 만드는지, 어디가 진짜 갈림길인지를 "
        "말하라. 여기만 읽어도 판이 보여야 하고, 항목 나열로 채우면 실패다, "
        "(2) 결정사항 — 번호매김, 명확·실행가능하게, "
        "(3) 결정 간 관계 — 어느 결정이 어느 결정의 전제인가, 상충하는 쌍은 무엇이고 그때 무엇을 "
        "우선하는가, 하나가 무너지면 함께 무너지는 묶음은 무엇인가. 독립이면 독립이라고 쓰라 — "
        "관계를 안 쓰면 실행 순서도 롤백 범위도 정할 수 없다, "
        "(4) 쟁점의 축과 소수의견 — 의견이 갈린 지점을 축으로 세우고, 갈린 이유를 셋으로 구분하라: "
        "(a) 사실을 몰라서 — 확보하면 닫힌다, 무엇을 확보할지 쓰라 / (b) 우선순위·가치가 달라서 — "
        "심의가 정할 수 없다, 결정권자에게 올려라 / (c) 같은 사실을 다르게 읽어서 — 더 따져야 한다, "
        "어느 해석이 어떤 가정에 기대는지 쓰라. 좌석이 명시한 non_negotiable(양보 불가 제약)과 stance 를 "
        "해당 축에 붙이고, 명시하지 않은 좌석은 '미표명'으로 기록하고 지어내지 마라. "
        "이 구분이 없으면 반대의견이 전부 '추후 검토'로 뭉개진다, "
        "(5) 뒤집히는 조건 — 무엇이 사실로 밝혀지면 위 결정이 바뀌는가를 결정마다 하나씩, 가능하면 "
        "수치 임계로. 아무것도 대지 못하는 결정은 근거가 아니라 취향이다, "
        "(6) 미해결 쟁점 + 담당·다음 액션, "
        "(7) 신뢰도·전제 — 이 결정문이 서 있는 전제와 그 전제가 깨지는 조건, "
        "(8) 수렴 경과 — 라운드를 거치며 입장이 어떻게 바뀌었는지. 마음을 바꾼 좌석과 그 계기를 "
        "적어라. 아무도 바꾸지 않았으면 그렇게 쓰라 — 그건 합의가 아니라 병렬 의견 수집이다. "
        "(1)(3)(4)(5)는 비워둘 수 없다 — 비면 항목 목록일 뿐 판단이 아니다.",
    "mechanism":
    ...
    "sim-plan":
        ...
        "(4) 솔버·도구 선택과 근거: 위 [사내 자산 현황] 의 [연결된 도구 앱] 에 있는 것에서 고르고, "
        "각 단계(전처리·솔버·실행·후처리)마다 무엇을 쓸지 지목하라. 오픈소스든 상용이든 그 목록에 "
        "있으면 후보다 — 이름을 아는 도구라도 목록에 없으면 '미연동' 으로 쓰고 연동·확보 경로를 "
        "제시하라. 보유 여부를 짐작해 쓰지 마라, "
        ...
        "(8) 검증(Validation)·유효범위·불확실성: 해석해·벤치마크·시험 대조에 더해 — 이 모델이 유효한 "
        "조건 범위(외삽 위험)와 정량 수용임계(무엇이 나오면 검증 통과인가)를 결과를 보기 전에 선언하라. "
        "Validation(올바른 방정식·모델인가를 현실 대조로 판정)은 코드검증((5))과 다르다. 핵심 예측은 "
        "파라미터 불확실을 전파해 점값이 아니라 구간으로 낸다, "
    ...
    "diagnosis":
        "원인 규명 결정문 8개 항목 — 불량/이상의 지배원인을 증거 위에서 좁힌다. 조치(고치기)는 이 심의의 "
        "산출이 아니다 — 원인과 미지영역까지가 산출이다. "
        "(1) 현상 정의: 무엇이·어디서·언제부터·얼마나(불량률·분포) 나는가를 관측값으로. 추정 섞지 마라, "
        "(2) is / is-not 경계(KT): 이 현상이 나는 조건과 바로 옆인데 안 나는 조건을 대비하라(로트·라인·온도·"
        "시점·자재). 경계가 원인 후보를 가장 많이 잘라낸다, "
        "(3) 결함 사슬(FTA): 최상위 사상에서 아래로 AND/OR 게이트로 원인 후보를 전개하라. 관측·물리로 배제 "
        "가능한 가지는 배제하고 근거를 남겨라, "
        "(4) 고장 양식(FMEA) 교차: 각 후보를 양식·영향·검출성으로 훑어 (3)에서 빠진 경로를 보완하라, "
        "(5) 가설 경합 표(ACH): 살아남은 원인 후보 × 증거의 부합/불일치 표를 만들어, 증거로 가장 덜 반박되는 "
        "후보를 지배원인으로. '가장 그럴듯한 하나'가 아니라 '가장 덜 반증된 것'이다, "
        "(6) cut set·기여도: 지배원인(단독/조합)과 상대 기여를 명시하라. 조합이면 어떤 인자들의 곱인지, "
        "(7) 반증 관측·미지영역: 이 결론이 틀렸다면 무엇이 관측되는가 + 아직 증거가 없어 못 가르는 영역을 "
        "명시하라. 여기가 다음 시험/해석의 입력이다, "
        "(8) 합의 근거·소수의견·신뢰도. (2)(5)(7)은 비워두지 마라 — 비면 근거 없이 원인을 단정한 것이다.",
    "option-select":
        "안 선택 결정문 8개 항목 — 경합하는 안 중에서 근거 위에서 고른다(못 고르면 무엇이 부족한지). "
        "(1) 결정 문제와 후보안: 무엇을 정하는가 + 후보안을 2개 이상 실명으로. 기준선(현행/기본안)을 반드시 "
        "하나 포함하라, "
        "(2) 평가 기준과 가중: 성능·원가·신뢰성·양산성·일정 등 기준을 정하고 가중을 합의하라. 가중 근거를 "
        "대라 — 임의 가중은 결론을 조작한다, "
        "(3) Pugh 1라운드: 기준선 대비 각 안을 기준별 +/0/− 로 채점하고 가중합을 내라. 점수의 근거(수치·시험·"
        "해석)를 각 칸에 붙여라, "
        "(4) 하이브리드안 생성: 상위안의 강점과 하위안의 강점을 결합한 새 안을 만들 수 있는가. 만들었으면 "
        "(5)에 넣어라, "
        "(5) Pugh 2라운드: 하이브리드 포함해 재채점. 라운드 간 순위가 바뀌었으면 왜인지 밝혀라, "
        "(6) Flip(민감도·뒤집힘 임계): 어떤 기준의 가중이나 어떤 입력이 얼마 바뀌면 1위가 뒤집히는가. 결론이 "
        "아슬아슬하면 그 인자를 먼저 확보하라, "
        "(7) 선택안과 조건: 고른 안 + 그것이 유효한 전제. 못 고르겠으면 무엇을 확보해야 갈리는지(→ 시험/해석), "
        "(8) 합의 근거·소수의견·신뢰도. (2)(3)(6)은 비워두지 마라 — 가중·채점·민감도가 없으면 취향일 뿐이다.",
    "credibility":
        "신뢰 판정문 8개 항목 — 이 해석/결정을 믿고 가도 되는지(go/no-go)를 채점한다. NASA-STD-7009 의 "
        "신뢰도 축을 쓴다. "
        "(1) 판정 대상과 결정: 무엇의 신뢰를 판정하며, 이 판정이 어떤 go/no-go 를 막고 있는가, "
        "(2) 신뢰도 축별 채점: 입력 데이터 품질·검증(Verification)·확인(Validation)·불확실성 정량화·모델 "
        "성숙도·인력/프로세스 — 각 축을 상/중/하로 채점하고 근거를 대라. 가장 낮은 축이 전체 신뢰의 상한이다, "
        "(3) 검증 증거: 이 결과가 해석해·벤치마크·시험과 대조된 적 있는가. 대조 없으면 '미검증'으로 명시하라"
        "(둘러대지 마라), "
        "(4) 불확실성·유효범위: 핵심 예측이 점값인가 구간인가. 이 결과가 유효한 조건 범위와 외삽 위험, "
        "(5) red-team 반박: 반대 지정석이 이 결론을 적극적으로 깨보라 — 가장 약한 가정·가장 위태로운 외삽·"
        "간과된 물리. 살아남았는가, "
        "(6) severe test: 이 모델이 틀렸다면 가장 먼저 깨질 예측은 무엇이고, 그 예측이 실제로 엄격한 시험을 "
        "통과했는가, "
        "(7) 판정: 생존(go) / 기각(no-go) / 조건부(무엇을 확보하면 go). 조건부면 조건을 수치로, "
        "(8) 합의 근거·소수의견·잔여 위험. (2)(5)(6)(7)은 비워두지 마라 — 채점·반박·시험 없는 '믿어도 "
        "된다'는 거수기다.",
    "build-plan":
        "구축 계획서 12개 항목 — 2단 해석 계획(sim-plan)을 고정 핵심으로 승계해, 한 번 돌리는 해석을 그 "
        "문제에 특화된 반복 실행형 파라메트릭 시뮬 툴로 \"구축\"하는 계획이다. 본체는 모델링 자동화 — 자산 "
        "3경로(a Dyna 낙하세트 승계 / b 제품 STEP 파트추출·메시 / c 2D 최소입력 슬라이스)가 하나의 정본 "
        "모델 IR 로 수렴하는 것이다. 각 항목의 구체성은 숫자·자산 경로·계약 필드로 채워라 — 비면 그 절은 "
        "계획서가 아니라 소망서다. 단위계는 export_dyna_cards 기본 ton_mm_s 로 통일한다. "
        "(1) 모듈 목표·비목표 [필수]: 이 툴이 반복적으로 답할 물리 질문 한 문장 + 대상 제품·파트 계열을 "
        "실명으로 + 최종 툴의 입력 스키마(파트명·디멘전·방향·재료세트·낙하고/속도·자세)와 출력 스키마(케이스별 "
        "max응력·에너지·파손플래그·스윕 결과표/응답면)를 필드 단위로 + 명시적 비목표(솔버를 LLM 으로 만들지 "
        "않는다·OSS 셋업 자동화가 아니다·이 물리 계열 밖은 안 한다). '낙하 해석 툴'로 끝나면 실패다. "
        "(2) 2단 sim-plan 10항목 승계·분류: context 에 [2단 sim_spec] 기계판독 규격이 있으면 그 "
        "parameters·outputs·model·compute 를 재파싱 없이 정본으로 승계하라(산문과 값이 어긋나면 규격을 "
        "우선하되 불일치를 (12)에 적는다). 2단 결정문 10항목과 양보 불가 조항을 표로 '고정 핵심 / "
        "파라미터화(스윕 변수) / 재검토'로 분류하라. 무엇이 얼어붙고(예: 솔버=oss-openradioss, "
        "차원축약=plane strain/axisym) 무엇이 변수인지 조용히 유실되지 않게. (2)의 지배방정식·차원축약은 "
        "(6)의 축약 가정과 물리적으로 일관해야 한다. "
        "(3) 파라미터 스윕·DOE [필수]: 스윕 변수 각각의 물리 의미·범위·수준 수(재료세트·기하 디멘전·낙하고/"
        "속도 v=√(2gh)·자세/방향·마찰)와 DOE 전략(전요인/부분요인/LHS), 총 케이스 수 N=수준의 곱(숫자), "
        "반응 지표. '여러 변수'는 실패, N 이 숫자로 나와야 한다. "
        "(4) 모델링 자동화 A — Dyna 낙하세트 승계 [필수·use-or-justify]: KooD3plotReader·"
        "export_dyna_cards(units=ton_mm_s)·upload_kfile 로 기준 덱을 읽어 PID→(SECID·MID·파트명·"
        "지오메트리) 승계 원장 매핑표를 값으로 확정. 파트 'Group\\Name'(예: Front\\Metal·PCB\\PCB·"
        "PKG\\PKG N·Bond\\Bond)을 정규식/사전으로 역할(하우징·기판·패키지·강체벽)에 매핑하고 스윕 대상 대 "
        "고정을 규칙표로 가른다(중복·익명 이름 방어). MAT_*_TITLE 을 물성 DB 에서 재발급해 원본값(RO·E·PR)과 "
        "자릿수까지 대조하고 유사매칭 오인을 사람이 승인해야 통과하는 게이트. CONTACT_TIED·BOUNDARY_SPC·"
        "INITIAL_VELOCITY 를 SET 단위로 승계하되 치수 스윕으로 파트가 바뀌면 무엇을 재생성/보존하는지 카드 "
        "단위로. 안 쓰면 왜 안 쓰는지 적어라. "
        "(5) 모델링 자동화 B — 제품 STEP 파트추출·메시 [필수·use-or-justify]: StepForge 로 어셈블리 "
        "하이라키 확정 후 대상 파트 추출, Dyna 파트명↔STEP 파트명 대조표(불일치 시 자동 진행 중단), "
        "인터페이스(tied/touching/clearance/침투) 검출을 Dyna CONTACT 와 교차검증(오분류는 사람 확정 게이트). "
        "메시는 BREP 경로로 파트별 크기·품질지표(minSICN)·요소형식을 선택 솔버와 정합, 치수 스윕 시 재메시 "
        "트리거 규정. 안 쓰면 이유. "
        "(6) 모델링 자동화 C — 2D 단면 최소입력 [필수·use-or-justify]: 정확히 3필드(part_name·dimension·"
        "direction)의 스키마·단위·좌표계(전역/파트로컬)를 못박아라. 방향→평면 normal 매핑, dimension 이 "
        "슬라이스 위치인지 두께인지, 원점 규칙. 자를 소스가 STEP(평면교차)인지 d3plot 지오메트리인지 정하고, "
        "2D 프로파일→쿼드 우선 메시→셸 요소로 잇고, plane strain/stress/axisym 축약을 (2)와 정합, "
        "두께방향 물성·적층 투영과 단면 검증 절차. 구체 케이스(예: PKG\\PKG N 을 Z중앙 XY평면 절단)를 "
        "적어라. 안 쓰면 이유. "
        "(7) 최소입력 계약·모델 IR 수렴·dry_run 게이트 [필수·크럭스]: 자산 3경로(a/b/c)가 수렴하는 단일 "
        "정본 모델 IR(파트·재료·연결·BC·메시)을 필드 단위로 정의하고 솔버 드라이버가 이 IR 만 소비함을 "
        "못박아라. 3경로가 서로 다른 산출을 내면 실패다. 각 경로의 최소입력이 자동 추론으로 '닫히는 조건' 대 "
        "'사람 게이트로 넘어가는 조건'을 경계선으로(sim-plan (7) 식별성의 구축판), 스윕 진입 전 비가역 "
        "dry_run 게이트 4종(중복/익명 이름 수, 재료 유사매칭 오인, 접촉 침투가 tied 로 새는지, 단위·밀도 "
        "자릿수 불일치)마다 검출 신호·임계값을 붙여 사람 승인 없이 스윕 진입 금지. 여기가 '모델링 자동화 최대 "
        "난제'를 실제로 닫는 자리다. "
        "(8) SW 아키텍처·자산 재사용·프로비넌스: 파이프라인(입력 어댑터 3경로→IR 빌더→형상·메시 코어→솔버 "
        "어댑터→후처리·지표 추출→스윕 오케스트레이터)을 이름 있는 컴포넌트로, 각 컴포넌트를 재사용"
        "(KooD3plotReader·StepForge·gmsh·oss-* 컨테이너·export_dyna_cards/upload_kfile/convert_file)/"
        "신규 로 표시, 인터페이스(포맷·좌표계·단위)를 명시. 승계 결정·물성 값의 출처와 입력 자산 해시·툴 "
        "버전을 고정. 재현 불가면 실패다. "
        "(9) 검증·회귀 3단 게이트 [필수]: g1 자동화 충실도(자동 생성 모델이 P1 골든을 허용오차 내 재현 — "
        "총질량·파트별 질량·무게중심·bbox·요소 수·접촉쌍 수를 원본 덱 1회 solve 의 d3plot 을 "
        "KooD3plotReader 로 읽은 baseline 과 수치 대조), g2 물리 수렴(에너지 밸런스·아워글래스·메시 "
        "독립성), g3 스윕 정합(단조성/추세·서러게이트 오차). 각 게이트에 지표·임계값을 숫자로(예: 골든 대비 "
        "max응력 오차 ≤5%). '동작하면'은 실패다. "
        "(10) 계산 규모·솔버 실행·오케스트레이션: 물리별 좌석 배정(명시적 낙하·충격→oss-openradioss, "
        "정적/암시→oss-calculix/oss-code-aster, 열→oss-elmer)과 실행 계약(convert_file·ntasks·라이선스·"
        "/data), 케이스당 코어시간×N=자원 예산, 제출 경로(slurm/STC), 벽시계·저장용량·일정·임계경로를 숫자로. "
        "(11) 페이즈 로드맵 P1→P4 + 게이트 [필수]: P1 단일 수동 골든(기준 덱 1건을 파트·재료·BC·oss 좌석 "
        "손수 지정·실행, d3plot+정량 기준셋), P2 모델링 자동화(자산 a/b/c→IR 로 신규 형상 1건을 "
        "최소입력만으로 자동 모델링해 P1 골든 재현), P3 파라메트릭 드라이버(스윕 N+응답면), P4 툴화(입출력 "
        "계약 동결·재현성 고정). 각 페이즈에 이름 있는 산출 아티팩트·진입 전제·다음 페이즈를 막는 수치 "
        "통과/실패 게이트를 배정하라. 게이트가 '동작하면'이면 실패, 수치여야 한다. "
        "(12) 자동화 한계 [필수]: 파트명 승계가 실패하는 형상·재료·연결(익명 MAT·이름 충돌·미등록), 얼린 "
...
_SIM_SPINE_VALIDATION = ("xd-model-validity", "xd-uncertainty")
...
_SIM_SPINE_VALIDATORS = _env_int("DELIB_SIM_SPINE_VALIDATION", 1)  # 1=검증·UQ 2석 포함(기본)
_SIM_FIXED_CAE = _SIM_FIXED_METHOD + (
    (_SIM_SPINE_CORE + (_SIM_SPINE_REVIEW if _SIM_SPINE_REVIEWERS else ())
     + (_SIM_SPINE_VALIDATION if _SIM_SPINE_VALIDATORS else ())) if _SIM_SPINE else ()
)
# 시험 계획 고정 좌석 — 이 다섯이 빠지면 계획서가 한쪽으로 기운다.
...
#   sim 상관     이 시험이 무슨 해석을 무슨 임계로 검증하는지 계약하는 쪽. 없으면 대조 대상 없는 시험이 된다
#   통계·신뢰성  표본수·검정력·산포를 보는 쪽. 없으면 근거 없는 표본수의 시험이 된다
_TEST_FIXED = ("rel-test-measurement", "xd-cae-modeling", "xd-program", "xd-model-correlation", "rel-reliability-stats")
...
# 각 값은 심의 BASE 에 덧붙는 지시 블록. hwax-deliberate.js MODIFIERS 와 키·취지 정합.
_MODIFIER_BLOCKS = {
    "voi": "[얹을 층 · 교착 정산(VoI)] 패널이 값으로 결론을 못 가르고 막히면, '누가 더 그럴듯한가'를 "
           "더 논쟁하지 말고 **무엇을 측정·계산하면 결론이 갈리는가**로 전환하라. 경합하는 결론마다 그것을 "
           "뒤집을 관측량을 지목하고, 그 관측의 (a) 결정 전환 가능성 (b) 확보 비용·시간 (c) 순이득(정보가치)을 "
           "견줘 **다음에 잴 것 하나**를 정하라. 이건 해석·시험 계획으로 넘어가는 다리다.",
    "premortem": "[얹을 층 · 사전부검] 결론을 굳히기 전에, '이 결정을 그대로 실행했더니 뒤에 크게 실패했다'고 "
                 "가정하고 **실패 경로부터** 3~5개 열거하라(성공 이유가 아니라). 각 실패 경로에 조기경보 신호와 "
                 "선제 방어책을 붙이고, 결론을 바꿔야 할 만큼 치명적인 것이 있으면 결정에 반영하라.",
    "toulmin": "[얹을 층 · 논증 엄밀(Toulmin)] 각 핵심 주장은 **주장(claim) → 근거(data) → 왜 그 근거가 그 "
               "주장을 지지하는가(warrant)**를 명시하라. warrant 가 비면 그 주장은 근거 없는 단정이다. 가능하면 "
               "반증조건(rebuttal)과 확신도(qualifier)를 덧붙여, 세 보이지만 밑이 빈 주장을 걸러내라.",
    "eliminative": "[얹을 층 · 완결 기준] '언제 이 심의가 끝인가'를 처음에 정하라. 결론을 무너뜨릴 수 있는 "
                   "반증요인(defeater)의 **유형**을 열거하고(빠진 도메인·미검증 가정·미확보 데이터·대안 가설), 각 "
                   "유형이 처리됐는지 점검하라. 남은 defeater 가 있으면 결론은 '조건부'이고, 무엇이 처리되면 종료인지 밝혀라.",
    "anon1r": "[얹을 층 · 익명 1R] 1라운드는 좌석들이 서로의 발언을 보기 전에 **독립적으로** 초기 입장·핵심 "
              "추정치를 낸다(초반 쏠림·거수기 방지). 2라운드부터 공개해 반박·수렴한다. 1라운드에서 추정이 크게 "
              "갈린 지점을 이후 라운드의 우선 쟁점으로 삼아라.",
}
...
def _resolve_opts(req_opts):
    ...
    if isinstance(req_opts, dict):
        ...
        if isinstance(md, list):
            ...
            for x in md:
                m = str(x).strip()
                if m in _ALLOWED_MODIFIERS and m not in seen:
                    seen.add(m)
                    picked.append(m)
...
async def _asset_snapshot(tools: dict, question: str) -> str:
    ...
    # 연결된 앱 전체를 먼저 싣는다. 아래 개별 조회는 지금 있는 도구 이름을 아는 것들이라,
    # 나중에 오픈소스 솔버 MCP·상용 시뮬레이션 MCP 가 붙어도 그쪽은 못 본다. 앱 목록은
    # 게이트웨이가 집계해 주므로 새 백엔드가 합류하면 코드 변경 없이 여기 나타난다 —
    # 좌석은 자유 조회(free_tools)로 그 도구를 직접 부를 수 있다.
    _apps = await _call(tools, "list_tool_apps", {"include_tools": False})
    if _apps:
        parts.append("[연결된 도구 앱 — 이 목록 밖의 도구는 이 심의에서 쓸 수 없다]\n"
                     + str(_apps)[:1400])
...
async def run_sim_deliberation(app, question: str, groups: list, req_opts=None, user: str = "",
    ...
    try:
        ...
        try:
            ...
            _tools_b = await _tools_by_name(app, groups, user=user, user_pat=user_pat)
...
async def run_test_plan(app, question: str, groups: list, req_opts=None, user: str = "",
    ...
    try:
        ...
        try:
            ...
            _tools = await _tools_by_name(app, groups, user=user, user_pat=user_pat)
...
async def _tidy_markdown(app, groups: list, history: list, note: str, user: str,
                         user_pat: str = "") -> str:
    ...
    try:
        ...
        agent = await _agent_for(app, groups, user=user, user_pat=user_pat)
...
async def run_report_save(app, note: str, history: list, groups: list, user: str = "",
    ...
    if _tidy:
        ...
        _md = await _tidy_markdown(app, groups, history, note, user, user_pat)
        if not _md:
