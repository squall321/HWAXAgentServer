...
EQ = {
    'Drums':               (60,  None,         0.0),   # 킥이 빠졌으니 저역은 필요 없다
    # ★킥 EQ. 최대 에너지가 90~120Hz(35.3%)에 몰려 '깊다'가 아니라 '때린다'로 들렸다.
    #  그 봉우리를 파고 50Hz 근처를 들어 올려 앞이 아니라 아래로 가게 한다.
    'Kick':                (28,  [(105, -6.0, 0.70), (52, 2.5, 0.60)], 0.0),
...
MIX = {
    ...
    'Drums':               (0.072, +0.00, 0.08),   # 스네어·림·탐·크래시 (킥은 따로)
    'Kick':                (0.105, +0.00, 0.015),  # 킥만 — 리버브는 거의 안 준다
...
def _mix_from_stems(inst, sf2, quiet):
    ...
    for name, a in stems.items():
        ...
        print(f'  {name:22s} RMS {rms:.4f} -> gain {g:5.2f}  팬 {pan:+.2f}  센드 {send:.2f}'
              f'  EQ hp{_hp}'
              + (' cut' + ','.join(f'{c[0]}Hz{c[1]:+.1f}' for c in
                                   ([_cut] if _cut and not isinstance(_cut[0], tuple) else _cut))
                 if _cut else '')
